...
func TestNewClient(t *testing.T) {
	c := NewJSONClient(&Options{"foo.com", "api_key", "agent", "application/json", 10, ""})

	if c.o == nil {
		t.Errorf("NewClient: unable to set client Options")
	}
...
func TestNewRequest(t *testing.T) {
	c := NewJSONClient(&Options{"foo.com", "api_key", "agent", "application/json", 10, "2017-10-17"})
...
func TestDo(t *testing.T) {
	...
	client := NewJSONClient(&Options{srv.URL, "api_key", "agent", "application/json", 10, ""})
	defer srv.Close()
	...
	badMethodReq, _ := client.NewRequest(context.Background(), "GET", "pass", nil)
	badMethodReq.Method = "NONSENSE"
...
func TestCheckError(t *testing.T) {
	tests := []struct {
		...
		{"StatusCodeError",
			&http.Response{
				Body:       ioutil.NopCloser(strings.NewReader(`{"error":{"message":"fail","status_code":500}}`)),
				StatusCode: http.StatusInternalServerError,
			},
			true,
			`{"error":{"message":"fail","status_code":500}}`,
		},
